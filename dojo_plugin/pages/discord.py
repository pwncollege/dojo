...
def oauth_url(user_id, *, secret=None):
    ...
    params = dict(client_id=DISCORD_CLIENT_ID, redirect_uri=url_for("discord.discord_redirect"), response_type="code", scope="identify", state=state)
    url = requests.Request("GET", f"{OAUTH_ENDPOINT}/authorize", params=params).prepare().url
    return url
...
def get_discord_id(auth_code):
    data = {
        ...
        "code": auth_code,
        "redirect_uri": url_for("discord.discord_redirect"),
    }
    headers = {
