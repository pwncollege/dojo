...
class AuthService {
	async login(credentials: LoginCredentials): Promise<AuthResponse> {
		const response = await dojoApiClient.post<AuthResponse>(
			"/auth/login",
			credentials,
		);
	...
	async resetPassword(
		...
		return dojoApiClient.post<{ success: boolean; message?: string }>(
	...
	async verifyEmail(
		...
		return dojoApiClient.get<{ success: boolean; message?: string }>(
			`/auth/verify/${token}`,
		);
	}
	...
	async updateProfile(data: any): Promise<{ success: boolean; user?: any }> {
		const response = await dojoApiClient.patch<{
			success: boolean;
			user?: any;
		}>("/users/me", data);
