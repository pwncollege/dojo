...
function processModule(
	...
	const resources: Resource[] = module.unified_items
		.filter(
			(item: any) => item.item_type === "resource" && item.type !== "header",
		)
		.map((item: any) => ({
			id: item.id,
...
class DojoService {
	...
	async createDojo(
		data: CreateDojoData,
	): Promise<{ success: boolean; dojo?: string; error?: string }> {
		return apiClient.post("/dojos/create", data);
	}

	// Get dojo modules
	async getDojoModules(dojoId: string): Promise<DojoModulesResponse> {
		const response = await apiClient.get<DojoModulesResponse>(
			`/dojos/${dojoId}/modules`,
		);
	...
	async getDojoDetail(dojoId: string): Promise<DojoDetailResponse> {
		try {
			...
			throw new Error(
				`Failed to fetch dojo details: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	}
	...
	async getDojoSolves(
		...
		username?: string,
		after?: string,
	): Promise<DojoSolvesResponse> {
		const params = new URLSearchParams();
		if (username) params.append("username", username);
		if (after) params.append("after", after);

		const query = params.toString();
		return apiClient.get<DojoSolvesResponse>(
			`/dojos/${dojoId}/solves${query ? "?" + query : ""}`,
		);
	}
	...
	async submitChallengeSolution(
		...
		return apiClient.post(
			`/dojos/${dojoId}/${moduleId}/${challengeId}/solve`,
			submission,
		);
	}
	...
	async getChallengeDescription(
		...
		return apiClient.get(
			`/dojos/${dojoId}/${moduleId}/${challengeId}/description`,
		);
	}
	...
	async submitSurveyResponse(
		...
		return apiClient.post(
			`/dojos/${dojoId}/${moduleId}/${challengeId}/surveys`,
			response,
		);
	}
	...
	async promoteAdmin(
		...
		return apiClient.post(`/dojos/${dojoId}/admins/promote`, {
			user_id: userId,
		});
	}
	...
	async pruneAwards(
		dojoId: string,
	): Promise<{ success: boolean; pruned_awards?: number }> {
	...
	async getCourseSolves(
		...
		const query = params.toString();
		return apiClient.get(
			`/dojos/${dojoId}/course/solves${query ? "?" + query : ""}`,
		);
	}
}
