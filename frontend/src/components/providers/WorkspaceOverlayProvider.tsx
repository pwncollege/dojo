...
export function WorkspaceOverlayProvider() {
	...
	const urlDojoId = isOnWorkspacePage
		? urlParts[urlParts.indexOf("dojo") + 1]
		: null;
	const urlModuleId = isOnWorkspacePage
		? urlParts[urlParts.indexOf("module") + 1]
		: null;
	const urlChallengeId = isOnWorkspacePage
		? urlParts[workspaceIndex + 2]
		: null;
	...
	useEffect(() => {
		...
		// Fetch dojo details
		dojoService
			.getDojoDetail(dojoId)
			.then((response) => {
				setDojoData(response.dojo);
				const module = response.dojo.modules.find(
					(m: any) => m.id === moduleId,
				);
				...
				const challenge = module?.challenges?.find(
					(c: any) => c.id === activeChallenge.challengeId,
				);
				if (challenge) {
					setActiveChallenge({
						...activeChallenge,
	...
	const handleWorkspaceClose = () => {
		...
		if (activeChallenge) {
			router.push(
				`/dojo/${activeChallenge.dojoId}/module/${activeChallenge.moduleId}`,
				{ scroll: false },
			);
		}
	};
