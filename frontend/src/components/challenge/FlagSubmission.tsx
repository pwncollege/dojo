	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (error?.status === 401 || error?.status === 403) {
				errorMessage =
					"Authentication required. Please log in to submit flags.";
			} else if (error?.status === 404) {
				errorMessage =
					"Challenge endpoint not found. This may be due to incorrect challenge mapping or the challenge not being properly configured in CTFd.";
			} else if (error?.status === 500) {
				errorMessage =
					"Server error. This might be due to authentication issues or the backend being unavailable.";
		...
		if (lastSubmission.result.success) {
			...
			if (lastSubmission.result.status === "authentication_required") {
				return (
					<Alert variant="destructive">
						...
						<AlertDescription>
							<strong>Authentication Required</strong> Please log in to the dojo
							platform to submit flags.
						</AlertDescription>
					</Alert>
				);
			}
			...
			return (
				<Alert variant="destructive">
					<AlertCircle className="h-4 w-4" />
					<AlertDescription>
						<strong>Incorrect!</strong> Flag "{lastSubmission.flag}" was
						rejected.
			...
			<CardContent className="space-y-4">
				...
				<form onSubmit={handleSubmit} className="space-y-4">
					...
					<div className="text-sm text-muted-foreground">
						Enter the flag you found for this challenge. Flags are typically in
						the format: pwn.college{"{...}"}
						<br />
						<strong>Note:</strong> You must be logged in to the dojo platform to
						submit flags.
					</div>
				</form>
			</CardContent>
