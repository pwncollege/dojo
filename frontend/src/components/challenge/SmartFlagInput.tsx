...
import { useStartChallenge, useSubmitChallengeSolution } from "@/hooks/useDojo";
...
export function SmartFlagInput({
	...
	const [initialClipboardValue, setInitialClipboardValue] =
		useState<string>("");
	const [isInitialized, setIsInitialized] = useState(false);
	const [clipboardSubmissionResult, setClipboardSubmissionResult] = useState<
		"fresh_success" | "already_solved" | "error" | null
	>(null);
	...
	const [popupState, setPopupState] = useState<
		"clipboard_detection" | "submission_result" | "regular_feedback" | null
	>(null);
	...
	const showPopupWithState = (
		state: "clipboard_detection" | "submission_result" | "regular_feedback",
	) => {
		setPopupState(state);
	...
	const { isMonitoring } = useClipboardFlagSubmission({
		...
		shouldProcessFlag: (flag: string) => {
			...
			const shouldNotProcess =
				dismissedFlags.has(flag) || flag === lastProcessedFlag;
		...
		<div className="relative">
			...
			>
						...
						className={cn(
							"relative flex items-center rounded-lg border h-9 px-3 gap-2 transition-all duration-200",
							{
								"border-muted-foreground/20 bg-muted/30 hover:bg-muted/50":
									status === "idle" && !isValidFlag,
								"border-primary/50 bg-primary/5 hover:bg-primary/10":
									isValidFlag && status === "idle",
				...
				<PopoverContent
					side="bottom"
					align="start"
					className={cn("w-auto min-w-80 max-w-md p-0 shadow-lg", {
						// Theme colors with opaque background + colored overlay
						"bg-background border relative":
							popupState === "clipboard_detection",
						"bg-background border-primary relative before:absolute before:inset-0 before:bg-primary/10 before:pointer-events-none":
							popupState === "submission_result" &&
							clipboardSubmissionResult !== "error",
						"bg-background border-destructive relative before:absolute before:inset-0 before:bg-destructive/10 before:pointer-events-none":
							popupState === "submission_result" &&
							clipboardSubmissionResult === "error",
						"bg-background border-primary relative before:absolute before:inset-0 before:bg-primary/10 before:pointer-events-none":
							popupState === "regular_feedback" && status === "success",
						"bg-background border-destructive relative before:absolute before:inset-0 before:bg-destructive/10 before:pointer-events-none":
							popupState === "regular_feedback" && status === "error",
					})}
					sideOffset={4}
					onInteractOutside={(e) => {
							...
							>
								...
								>
									...
									<span className="text-sm font-medium text-accent">
										Flag detected in clipboard
									</span>
								</motion.div>

								<motion.div
									variants={itemVariants}
									className="text-xs text-muted-foreground mb-4 font-mono bg-muted/80 p-3 rounded border break-all min-w-0"
								>
									{clipboardFlag}
								</motion.div>
							...
							>
								...
								>
									<motion.div
										initial={{
											scale: 0,
											rotate: clipboardSubmissionResult === "error" ? -90 : 0,
										}}
								...
								>
									{clipboardSubmissionResult === "error"
										? message ||
											"The flag you submitted is incorrect. Please try again."
										: clipboardSubmissionResult === "already_solved"
								...
								>
									...
									<motion.div variants={buttonVariants}>
										<Button
											...
											className={
												status === "error"
													? "text-destructive hover:text-destructive hover:bg-destructive/10 focus-visible:ring-destructive"
													: ""
											}
										>
											Dismiss
