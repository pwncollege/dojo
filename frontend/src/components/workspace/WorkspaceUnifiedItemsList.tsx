...
export function WorkspaceUnifiedItemsList({
	...
	if (apiUnifiedItems && apiUnifiedItems.length > 0) {
		...
		if (resources) {
			resources.forEach((resource) => {
				items.push([
					...
					{ ...resource, item_type: "resource" as const },
				]);
			});
		}
		...
		challenges.forEach((challenge) => {
			const index =
				challenge.unified_index !== undefined
	...
	const sections: Array<{
		header: string;
		items: (APIUnifiedItem | UnifiedItem)[];
	}> = [];
							...
							if (
								...
								return (
									<div
										...
										onClick={() =>
											!isActive && onChallengeStart(moduleId, item.id)
										}
									>
