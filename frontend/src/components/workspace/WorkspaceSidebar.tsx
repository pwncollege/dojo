		...
		<div className="bg-background flex flex-col h-full w-full">
			...
			<div className="border-b px-6 py-3 animate-in fade-in slide-in-from-top-2 duration-300 delay-100">
				...
				>
										...
										{(() => {
											...
											const materialsCount = module.unified_items
												...
												: module.resources?.filter(
														(r) =>
															r.type === "lecture" || r.type === "markdown",
													).length || 0;
					...
					<div
						className={`flex items-center ${sidebarCollapsed ? "flex-col gap-1 py-[4px] h-[41px]" : "gap-0.5"} flex-shrink-0`}
					>
						{!sidebarCollapsed && (
						...
						{(() => {
							...
							finalUnifiedItems.forEach((item) => {
								if (item.item_type === "resource" && item.type === "header") {
									...
									currentSection =
										item.content || item.name || "Untitled Section";
									currentItems = [];
								} else {
							...
							if (sections.length === 0) {
								const resourceItems = finalUnifiedItems.filter(
									(item) =>
										item.item_type === "resource" && item.type !== "header",
								);
								const challengeItems = finalUnifiedItems.filter(
									(item) => item.item_type === "challenge",
								);
								...
								>
									...
									{section.items.map((item, index) => {
										if (
											...
											const challengeIndex = module.challenges.findIndex(
												(c) => c.id === item.id,
											);
												...
												<Popover key={`${item.id}-${index}`}>
													...
													>
														<ChallengePopoverContent
															...
															isActive={isActive}
															onStartChallenge={() =>
																onChallengeStart(module.id, item.id)
															}
															isPending={isPending}
														/>
													</PopoverContent>
