...
import { useAuthStore, useDojoStore } from "@/stores";
...
export function DojoPageClient({ dojo, dojoId }: DojoPageClientProps) {
	...
	const stats = useMemo(
		() => ({
			totalChallenges:
				dojo.stats?.challenges ||
				dojo.modules.reduce(
					(acc, mod) => acc + (mod.challenges?.length || 0),
					0,
				),
			totalSolves: dojo.stats?.solves || solves.length,
			uniqueHackers:
				dojo.stats?.users || new Set(solves.map((solve) => solve.user_id)).size,
	...
	const getDojoIcon = (dojo: DojoDetail) => {
		...
		if (dojo?.award?.belt && dojo.official) {
			return (
				<Belt color={dojo.award.belt} className="h-8 w-auto max-w-[72px]" />
			);
		}
				...
				<div className="mb-12 relative">
					...
					<div className="pr-16">
						...
						<div className="flex items-center gap-3 mb-4">
							{dojo?.official && <Badge variant="default">Official</Badge>}
							<Badge variant="outline">{stats.totalSolves} Total Solves</Badge>
							<Badge variant="secondary">
								{dojo.modules.length}{" "}
								{dojo.modules.length === 1 ? "Module" : "Modules"}
							</Badge>
						</div>
					...
					<div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
						{dojo.modules.map((module, index) => {
							const moduleProgress = module.challenges
								? (module.challenges.filter((c) => solvedChallengeIds.has(c.id))
										.length /
							...
							return (
								<Card
									key={module.id}
									className="h-full hover:border-muted-foreground/20 transition-all duration-200 hover:translate-y-[-2px] hover:shadow-md"
								>
									{/* Module Card Header */}
