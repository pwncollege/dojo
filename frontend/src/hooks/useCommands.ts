	...
	return useMemo(() => {
		...
		if (activeChallenge) {
			...
			const currentModule = modules?.find(
				(m) => m.id === activeChallenge.moduleId,
			);
			const currentChallengeIndex =
				currentModule?.challenges.findIndex(
					(c) => c.id === activeChallenge.challengeId,
				) ?? -1;

			if (currentModule && currentChallengeIndex > 0) {
				const previousChallenge =
					currentModule.challenges[currentChallengeIndex - 1];
				commands.push({
			...
			if (
				currentModule &&
				currentChallengeIndex < currentModule.challenges.length - 1
			) {
				const nextChallenge =
					currentModule.challenges[currentChallengeIndex + 1];
				commands.push({
			...
			commands.push({
				...
				action: () => {
					...
					const flagInput = document.querySelector(
						'input[placeholder*="flag"], input[type="text"]',
					) as HTMLInputElement;
					if (flagInput) {
						flagInput.focus();
						flagInput.select();
