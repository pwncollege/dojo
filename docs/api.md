...
1. dates - an empty dictionary.
2. users - an empty dictionary.
3. ranks - an empty dictionary.
...
This parts is responsible for querying the database to get the belts. It returns all non hidden users with a
...
After that, we sort the belts from last to first (black to orange), and from oldest to newest.
...
entry in the `users` dictionary for the current belt's owner. If we don't, we create a new list in the
`users` dictionary with `belt.user_id` as the name. That list will contain `belt.handle`, `belt.site`,
