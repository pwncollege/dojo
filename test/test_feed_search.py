...
def test_feed_events_limit_and_offset_handling(example_dojo, random_user):
    ...
    invalid = fetch_feed(limit="abc", offset="xyz")
    assert invalid["meta"]["limit"] == 50
    assert invalid["meta"]["offset"] == 0

    page = fetch_feed(limit=1, offset=1)
    assert page["meta"]["limit"] == 1
    assert page["meta"]["offset"] == 1
    assert page["meta"]["count"] == len(page["data"]) <= 1
...
def test_feed_events_readable_anonymously_and_persist(example_dojo, random_user):
    ...
    anonymous_ids = [e["id"] for e in fetch_feed(session=requests.Session(), limit=100)["data"]]
    authenticated_ids = [e["id"] for e in fetch_feed(session=session, limit=100)["data"]]
...
def test_feed_page_shows_live_activity(random_user_browser, random_user, example_dojo):
    name, session = random_user
    random_user_browser.get(FEED_PAGE_URL)
    assert name not in random_user_browser.find_element("tag name", "body").text

    solve_challenge_offline(example_dojo, "hello", "apple", session=session, user=name)
    wait_for_background_worker()

    WebDriverWait(random_user_browser, 20).until(
        lambda browser: name in browser.find_element("tag name", "body").text
    )
...
def test_search_trims_whitespace_and_ignores_case(feed_official_dojo, admin_session):
    ...
    baseline = search_results(marker, session=admin_session)
    normalized = search_results(f"   {marker.upper()}   ", session=admin_session)
    for kind in ["dojos", "modules", "challenges"]:
        assert sorted(item["id"] for item in baseline[kind]) == sorted(item["id"] for item in normalized[kind])
...
def test_search_result_shapes_and_links(feed_official_dojo, admin_session):
    ...
    assert {"id", "name", "link", "description"} <= set(dojo)
    ...
    assert {"id", "name", "dojo", "link", "description"} <= set(module)
    assert {"id", "name", "link"} <= set(module["dojo"])
    ...
    assert {"id", "name", "module", "dojo", "link", "description"} <= set(challenge)
    ...
    assert any(d["id"] == reference_id for d in search_results("FSDOJODESC", admin_session)["dojos"])
    assert any(m["dojo"]["id"] == reference_id for m in search_results("FSMODULEDESC", admin_session)["modules"])
    assert any(
        c["dojo"]["id"] == reference_id
        for c in search_results("FSCHALLENGEDESC", admin_session)["challenges"]
    )
...
def test_search_hides_private_dojos_until_joined(feed_hidden_dojo, admin_session, random_user):
    ...
    results = search_results(marker, session=session)
...
def test_search_respects_visibility_windows(visibility_test_dojo, random_user, admin_session, guest_dojo_admin):
    name, session = random_user
    assert session.get(f"{DOJO_URL}/dojo/{visibility_test_dojo}/join/").status_code == 200
    admin_name, dojo_admin_session = guest_dojo_admin
    assert dojo_admin_session.get(f"{DOJO_URL}/dojo/{visibility_test_dojo}/join/").status_code == 200
    promoted = admin_session.post(
        f"{DOJO_URL}/pwncollege_api/v1/dojos/{visibility_test_dojo}/admins/promote",
        json={"user_id": get_user_id(admin_name)},
    )
    assert promoted.status_code == 200 and promoted.json()["success"], promoted.text[:200]
    ...
    admin_modules = search_results("Module 1", session=dojo_admin_session)["modules"]
    assert [m for m in admin_modules if m["dojo"]["id"] == visibility_test_dojo and m["id"] == "module1"], \
        "dojo administrators must be able to search future modules they manage"

    for challenge_name, challenge_id in [("Challenge A", "challenge-a"), ("Challenge B", "challenge-b")]:
        admin_challenges = search_results(challenge_name, session=dojo_admin_session)["challenges"]
        assert [c for c in admin_challenges
                if c["dojo"]["id"] == visibility_test_dojo and c["id"] == challenge_id], \
            f"dojo administrators must be able to search hidden challenge {challenge_id}"


def test_search_hides_challenges_of_hidden_modules(
    hidden_challenges_dojo, admin_session, random_user, guest_dojo_admin
):
    name, session = random_user
    ...
    admin_name, dojo_admin_session = guest_dojo_admin
    assert dojo_admin_session.get(f"{DOJO_URL}/dojo/{hidden_challenges_dojo}/join/").status_code == 200
    promoted = admin_session.post(
        f"{DOJO_URL}/pwncollege_api/v1/dojos/{hidden_challenges_dojo}/admins/promote",
        json={"user_id": get_user_id(admin_name)},
    )
    assert promoted.status_code == 200 and promoted.json()["success"], promoted.text[:200]
    admin_results = search_results("CHALLENGE", session=dojo_admin_session)
    assert [c for c in admin_results["challenges"] if c["dojo"]["id"] == hidden_challenges_dojo], \
        "dojo administrators must be able to search challenges hidden from members"
