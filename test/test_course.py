...
def test_as_user_is_ignored_for_non_admins(course, identity_budget):
    student_name, student_session = new_user()
    student_id = get_user_id(student_name)
    assert patch_identity(student_session, course.dojo, course.a).json()["success"] is True
    ...
    try:
        response = plain_session.post(f"{API}/docker", json=dict(
            dojo=course.dojo, module="hello", challenge="apple", as_user=student_id))
        assert response.status_code == 200 and response.json()["success"] is True, response.text[:200]
        flag = workspace_run("cat /flag", user=plain_name, root=True).stdout.strip()
        assert flag == challenge_flag(course.dojo, "hello", "apple", user=plain_name), \
            "as_user must be ignored for non-admins: the workspace is their own"
    finally:
        remove_workspace_container(plain_name)
