...
def test_slow_query_logging(random_user_session):
    ...
    result = dojo_run("sh", "-c", "docker logs --since 10m ctfd 2>&1 | grep 'Slow query' | tail -5")
    logs = result.stdout
...
def test_fast_query_not_logged(random_user_session):
    dojo_run("sh", "-c", "docker logs --since 10m ctfd 2>&1 | grep 'Slow query' | wc -l > /tmp/before_count")
    ...
    dojo_run("sh", "-c", "docker logs --since 10m ctfd 2>&1 | grep 'Slow query' | wc -l > /tmp/after_count")
