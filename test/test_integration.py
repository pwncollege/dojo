...
def test_start(random_user):
    name, _ = random_user
    starting_test(name, False)

@container_running
def test_privileged(random_user):
    name, _ = random_user
    starting_test(name, True)

def restarting_test(name, swap):
    token = container_token(name)
    start_info = container_info(name)
    ...
    restart_info = container_info(name)
...
def test_int_restart(random_user):
    name, _ = random_user
    restarting_test(name, False) # norm -> norm
    restarting_test(name, True)  # norm -> priv
    restarting_test(name, False) # priv -> priv
    restarting_test(name, True)  # priv -> norm

@container_running
def test_int_list(random_user):
    name, _ = random_user
    token = container_token(name)
    headers = {"type": "container", "token": token}
...
@container_running
def test_int_list_priv(random_user):
    pass

@container_running
def test_int_info(random_user):
    pass
