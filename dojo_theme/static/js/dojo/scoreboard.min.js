function loadScoreboard(div_name, server_name, page) {
    ...
    var endpoint = `/pwncollege_api/v1/scoreboard/${server_name}`;
    if (page != null)
        endpoint += `/${page - 1}`;
    ...
    CTFd.fetch(endpoint, {
        ...
        if (page != null) {
            const scoreboardPages = $(`#scoreboard-${div_name}-pages`);
            scoreboardPages.empty();
            const minPage = Math.max(1, page - 5);
            const maxPage = Math.min(page + 5, result.num_pages);
            for (let i = minPage; i <= maxPage; i++) {
                const pageButton = $(`
                <li class="scoreboard-page"><a href="javascript:loadScoreboard('${div_name}', '${server_name}', ${i})">${i}</a></li>
                `);
...
$(function() {
    loadScoreboard("weekly", "weekly");
    loadScoreboard("overall", "semester", 1);
});
